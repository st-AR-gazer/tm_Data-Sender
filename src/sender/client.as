namespace DataSender {
    namespace Sender {
        ...
        class WebSocketClient {
            ...
            WebSocketClient(const string &in url) {
                ParseUrl(url);
            }
            ...
            bool Conn(const string &in h, uint16 p) {
                return usingTLS ? ssock.Connect(h, p) : sock.Connect(h, p);
            }
            bool Ready() {
                return usingTLS ? !ssock.Connecting() : sock.IsReady();
            }
            int Avail() {
                return usingTLS ? ssock.Available() : sock.Available();
            }
            string Read(int n) {
                return usingTLS ? ssock.ReadRaw(n) : sock.ReadRaw(n);
            }
            bool Hung() {
                return usingTLS ? false : sock.IsHungUp();
            }

            void CloseSock() {
                if (usingTLS) {
                    if (ssock !is null) ssock.Close();
                } else {
            ...
            void ParseUrl(const string &in url) {
                ...
                port = uint16(colon < 0 ? (secure ? 443 : 80) : Text::ParseUInt(auth.SubStr(colon + 1)));
            ...
            void WriteByte(MemoryBuffer@ b, uint8 v) {
                b.Write(v);
            }
            ...
            MemoryBuffer@ FrameText(const string &in payload) {
                ...
                for (uint i = 0; i < n; i++) {
                    uint8 m = (i & 3) == 0 ? k0 : (i & 3) == 1 ? k1 : (i & 3) == 2 ? k2 : k3;
